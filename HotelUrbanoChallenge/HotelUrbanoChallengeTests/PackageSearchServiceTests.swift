//
//  PackageSearchServiceTests.swift
...
final class PackageSearchServiceTests: XCTestCase {
    
    private var httpClientMock: HttpClientMock!
    private var service: PackageSearchService!
    
    override func setUpWithError() throws {
        self.httpClientMock = HttpClientMock()
        self.service = PackageSearchService(httpClient: self.httpClientMock)
    }
    ...
    func testQueryGeneration() throws {
        self.service.search(in: "Location Value") { _ in }
        XCTAssertEqual(self.httpClientMock.lastUrl?.absoluteString, "https://www.hurb.com/search/api?q=location%20value&filters=is_offer%7C1&page=1")
        
        self.service.search(in: "  Location Value  ") { _ in }
        XCTAssertEqual(self.httpClientMock.lastUrl?.absoluteString, "https://www.hurb.com/search/api?q=location%20value&filters=is_offer%7C1&page=1")
        
        self.service.search(in: "Invalid&Characters") { _ in }
        XCTAssertEqual(self.httpClientMock.lastUrl?.absoluteString, "https://www.hurb.com/search/api?q=invalid%26characters&filters=is_offer%7C1&page=1")
        
        self.service.search(in: "Invalid|Characters") { _ in }
        XCTAssertEqual(self.httpClientMock.lastUrl?.absoluteString, "https://www.hurb.com/search/api?q=invalid%7Ccharacters&filters=is_offer%7C1&page=1")
        
        self.service.search(in: "Invalid=Characters") { _ in }
        XCTAssertEqual(self.httpClientMock.lastUrl?.absoluteString, "https://www.hurb.com/search/api?q=invalid%3Dcharacters&filters=is_offer%7C1&page=1")
    }
    
    func testParseResponse() throws {
        guard let jsonPath = Bundle.init(for: type(of: self)).path(forResource: "package", ofType: "json") else {
            XCTFail()
            return
        }
        ...
        self.service.search(in: "Location Value") { result in
            ...
            case .success(let response):
                XCTAssertEqual(response.results.count, 18)
            case .failure(_):
                XCTFail()
            }
