...
final class SuggestionsServiceTests: XCTestCase {
    ...
    private var service: SuggestionsService!
    
    override func setUpWithError() throws {
        self.httpClientMock = HttpClientMock()
        self.service = SuggestionsService(httpClient: self.httpClientMock)
    }
    ...
    func testRequest() throws {
        self.service.fetch("Location Value") { _ in }
    ...
    func testQueryGeneration() throws {
        self.service.fetch("Location Value") { _ in }
        XCTAssertEqual(self.httpClientMock.lastUrl?.absoluteString, "https://www.hurb.com/search/api/suggestion?q=location%20value")
        
        self.service.fetch("  Location Value  ") { _ in }
        XCTAssertEqual(self.httpClientMock.lastUrl?.absoluteString, "https://www.hurb.com/search/api/suggestion?q=location%20value")
        
        self.service.fetch("Invalid&Characters") { _ in }
        XCTAssertEqual(self.httpClientMock.lastUrl?.absoluteString, "https://www.hurb.com/search/api/suggestion?q=invalid%26characters")
        
        self.service.fetch("Invalid|Characters") { _ in }
        XCTAssertEqual(self.httpClientMock.lastUrl?.absoluteString, "https://www.hurb.com/search/api/suggestion?q=invalid%7Ccharacters")
        
        self.service.fetch("Invalid=Characters") { _ in }
        XCTAssertEqual(self.httpClientMock.lastUrl?.absoluteString, "https://www.hurb.com/search/api/suggestion?q=invalid%3Dcharacters")
    }
    
    func testParseResponse() throws {
        guard let jsonPath = Bundle.init(for: type(of: self)).path(forResource: "suggestion", ofType: "json") else {
            XCTFail()
            return
        }
        ...
        self.service.fetch("Location Value") { result in
            switch result {
            case .success(let response):
                XCTAssertEqual(response.suggestions.count, 5)
            case .failure(_):
                XCTFail()
            }
